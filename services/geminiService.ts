...
const createShiftTool: FunctionDeclaration = {
    name: 'createShift',
    description: 'Create a new work shift for an employee.',
    parameters: {
        type: Type.OBJECT,
        properties: {
            employee_id: { type: Type.STRING, description: 'The UUID of the employee' },
            start_time: { type: Type.STRING, description: 'Start time in ISO format (YYYY-MM-DDTHH:mm:ss)' },
            end_time: { type: Type.STRING, description: 'End time in ISO format (YYYY-MM-DDTHH:mm:ss)' },
            location_id: { type: Type.STRING, description: 'The UUID of the location' },
            type: { type: Type.STRING, description: 'Type of shift: work, off, vacation, sick, permission' }
        },
...
export interface AIResponse {
    action: 'createTask' | 'createIncident' | 'createShift' | 'none';
    data?: any;
    message: string;
}
...
export const processNaturalLanguageCommand = async (
    ...
    try {
        ...
        let roomMapping = '';
        if (context.rooms) {
            roomMapping = context.rooms.map(r => {
        ...
        Your capability is to CREATE TASKS, REPORT INCIDENTS, and CREATE SHIFTS based on user input.
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                tools: [{ functionDeclarations: [createTaskTool, createIncidentTool, createShiftTool] }],
            }
        });
        ...
        if (candidates && candidates.length > 0) {
            ...
            for (const part of parts) {
                if (part.functionCall) {
                    ...
                    return {
                        action: fc.name as 'createTask' | 'createIncident' | 'createShift',
                        data: fc.args,
